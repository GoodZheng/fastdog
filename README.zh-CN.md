...
FastDog 是一款基于 **WPF + ripgrep** 打造的 Windows 桌面全文搜索工具，参考 grepWin 的核心搜索理念，专为开发者和团队在代码库、日志、文档间快速检索内容而设计。

软件内置 [ripgrep](https://github.com/BurntSushi/ripgrep) 14.1.1——当前最快的文本搜索引擎，并通过 `--json` 模式以流式推送结果，能在海量文件中瞬间定位匹配内容。无论是日常代码检索、日志排查，还是替换前的批量预览，FastDog 都能提供极速、精准且可视化的搜索体验。

**核心能力：**

- 🔎 支持纯文本与正则表达式搜索，可自由组合大小写敏感、全词匹配等精准条件
- 🧰 多维度过滤：文件名通配符（`*.cs;*.txt`）、目录排除、按修改日期范围筛选
- 🖥️ 三栏式结果视图——文件列表 + 匹配行列表 + 语法高亮代码预览，点击即定位
- 💾 搜索历史持久化与会话恢复，下次启动自动还原上次的搜索条件
- ⚡ 布局持久化、拖放设置路径、双击打开/跳转、右键复制路径等贴心细节
- 📦 单文件可执行，零配置开箱即用
...
- **📌 系统托盘** — 关闭即驻留托盘，后台常驻、随用随调
- **⌨️ 输入补全** — 搜索路径与内容自动提示历史输入，快速复用
...
- **ℹ️ 关于窗口（About Dialog）** — 新增「关于」窗口，展示应用名称、版本号、技术栈及相关链接（如仓库地址、开源许可证）
- **🔄 检查更新（Check for Updates）** — 新增「检查更新」功能，检测并向用户提示新版本发布
